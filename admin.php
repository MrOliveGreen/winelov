	...
	switch ($_GET['view']) {
        ...
        case 'wines':
            ...
            switch ($_GET['sub_view']) {
                case '':
                    // page header
                    page_header($control_panel['title']." &rsaquo; ".__("Wines"));

                    // get insights
                    $insights = [];
                    /* total wines */
                    $get_wines = $db->query("SELECT COUNT(*) as count FROM wines") or _error("SQL_ERROR");
                    $insights['wines'] = $get_wines->fetch_assoc()['count'];

                    // get data
                    require('includes/class-pager.php');
                    $params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
                    $params['total_items'] = $insights['wines'];
                    $params['items_per_page'] = $system['max_results'];
                    $params['url'] = $system['system_url'].'/'.$control_panel['url'].'/wines?page=%s';
                    $pager = new Pager($params);
                    $limit_query = $pager->getLimitSql();
                    $get_rows = $db->query("SELECT * FROM wines ORDER BY Id_item ASC ".$limit_query) or _error("SQL_ERROR");
                    if($get_rows->num_rows > 0) {
                        while($row = $get_rows->fetch_assoc()) {
                            $rows[] = $row;
                        }
                ...
                case 'find':
                    ...
                    require('includes/class-pager.php');
                    $params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
                    $total = $db->query(sprintf('SELECT COUNT(*) as count FROM wines WHERE (name LIKE %1$s OR winery LIKE %1$s OR region LIKE %1$s OR country LIKE %1$s OR grapes LIKE %1$s OR foodpairing LIKE %1$s) ORDER BY name ASC', secure($_GET['query'], 'search') )) or _error("SQL_ERROR");
                    $params['total_items'] = $total->fetch_assoc()['count'];
                    $params['items_per_page'] = $system['max_results'];
                    $params['url'] = $system['system_url'].'/'.$control_panel['url'].'/wines/find?query='.$_GET['query'].'&page=%s';
                    $pager = new Pager($params);
                    $limit_query = $pager->getLimitSql();
                    $get_rows = $db->query(sprintf('SELECT * FROM wines WHERE (name LIKE %1$s OR winery LIKE %1$s OR region LIKE %1$s OR country LIKE %1$s OR grapes LIKE %1$s OR foodpairing LIKE %1$s) '.$limit_query, secure($_GET['query'], 'search') )) or _error("SQL_ERROR");
                    if($get_rows->num_rows > 0) {
                        while($row = $get_rows->fetch_assoc()) {
                            $rows[] = $row;
                        }
                ...
                case 'edit':
                    ...
                    // get data
                    $get_data = $db->query(sprintf("SELECT * FROM wines WHERE Id_item = %s ", secure($_GET['id'], 'int') )) or _error("SQL_ERROR");
                    if($get_data->num_rows == 0) {
                        _error(404);
                    }
                    $data = $get_data->fetch_assoc();
                    ...
                    // page header
                    page_header($control_panel['title']." &rsaquo; ".__("Wines")." &rsaquo; ".$data['name']);
                    break;
                ...
                case 'add':
                    ...
                    // page header
                    page_header($control_panel['title']." &rsaquo; ".__("Wines")." &rsaquo; Add");
                    break;
        ...
        case 'wineries':
            ...
            switch ($_GET['sub_view']) {
                case '':
                    // page header
                    page_header($control_panel['title']." &rsaquo; ".__("Wineries"));

                    // get insights
                    $insights = [];
                    /* total wineries */
                    $get_wineries = $db->query("SELECT COUNT(*) as count FROM winery") or _error("SQL_ERROR");
                    $insights['wineries'] = $get_wineries->fetch_assoc()['count'];

                    // get data
                    require('includes/class-pager.php');
                    $params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
                    $params['total_items'] = $insights['wineries'];
                    $params['items_per_page'] = $system['max_results'];
                    $params['url'] = $system['system_url'].'/'.$control_panel['url'].'/wineries?page=%s';
                    $pager = new Pager($params);
                    $limit_query = $pager->getLimitSql();
                    $get_rows = $db->query("SELECT * FROM winery ORDER BY ID ASC ".$limit_query) or _error("SQL_ERROR");
                    if($get_rows->num_rows > 0) {
                        while($row = $get_rows->fetch_assoc()) {
                            $rows[] = $row;
                        }
                ...
                case 'find':
                    ...
                    require('includes/class-pager.php');
                    $params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
                    $total = $db->query(sprintf('SELECT COUNT(*) as count FROM winery WHERE (Name LIKE %1$s OR email LIKE %1$s OR website LIKE %1$s OR street LIKE %1$s OR city LIKE %1$s OR state LIKE %1$s OR Region LIKE %1$s OR Country LIKE %1$s) ORDER BY Name ASC', secure($_GET['query'], 'search') )) or _error("SQL_ERROR");
                    $params['total_items'] = $total->fetch_assoc()['count'];
                    $params['items_per_page'] = $system['max_results'];
                    $params['url'] = $system['system_url'].'/'.$control_panel['url'].'/wineries/find?query='.$_GET['query'].'&page=%s';
                    $pager = new Pager($params);
                    $limit_query = $pager->getLimitSql();
                    $get_rows = $db->query(sprintf('SELECT * FROM winery WHERE (Name LIKE %1$s OR email LIKE %1$s OR website LIKE %1$s OR street LIKE %1$s OR city LIKE %1$s OR state LIKE %1$s OR Region LIKE %1$s OR Country LIKE %1$s)'.$limit_query, secure($_GET['query'], 'search') )) or _error("SQL_ERROR");
                    if($get_rows->num_rows > 0) {
                        while($row = $get_rows->fetch_assoc()) {
                            $rows[] = $row;
                        }
                ...
                case 'edit':
                    ...
                    // get data
                    $get_data = $db->query(sprintf("SELECT * FROM winery WHERE ID = %s ", secure($_GET['id'], 'int') )) or _error("SQL_ERROR");
                    if($get_data->num_rows == 0) {
                        _error(404);
                    }
                    $data = $get_data->fetch_assoc();
                    ...
                    // page header
                    page_header($control_panel['title']." &rsaquo; ".__("Wineries")." &rsaquo; ".$data['Name']);
                    break;
                ...
                case 'add':
                    ...
                    // page header
                    page_header($control_panel['title']." &rsaquo; ".__("Wineries")." &rsaquo; Add");
                    break;
        ...
        case 'regions':
            ...
            switch ($_GET['sub_view']) {
                case '':
                    // page header
                    page_header($control_panel['title']." &rsaquo; ".__("Regions"));

                    // get insights
                    $insights = [];
                    /* total regions */
                    $get_regions = $db->query("SELECT COUNT(*) as count FROM regions") or _error("SQL_ERROR");
                    $insights['regions'] = $get_regions->fetch_assoc()['count'];

                    // get data
                    require('includes/class-pager.php');
                    $params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
                    $params['total_items'] = $insights['regions'];
                    $params['items_per_page'] = $system['max_results'];
                    $params['url'] = $system['system_url'].'/'.$control_panel['url'].'/regions?page=%s';
                    $pager = new Pager($params);
                    $limit_query = $pager->getLimitSql();
                    $get_rows = $db->query("SELECT * FROM regions ORDER BY id ASC ".$limit_query) or _error("SQL_ERROR");
                    if($get_rows->num_rows > 0) {
                        while($row = $get_rows->fetch_assoc()) {
                            $rows[] = $row;
                        }
                ...
                case 'find':
                    ...
                    require('includes/class-pager.php');
                    $params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
                    $total = $db->query(sprintf('SELECT COUNT(*) as count FROM regions WHERE (country_id LIKE %1$s OR name LIKE %1$s OR headline LIKE %1$s) ORDER BY name ASC', secure($_GET['query'], 'search') )) or _error("SQL_ERROR");
                    $params['total_items'] = $total->fetch_assoc()['count'];
                    $params['items_per_page'] = $system['max_results'];
                    $params['url'] = $system['system_url'].'/'.$control_panel['url'].'/regions/find?query='.$_GET['query'].'&page=%s';
                    $pager = new Pager($params);
                    $limit_query = $pager->getLimitSql();
                    $get_rows = $db->query(sprintf('SELECT * FROM regions WHERE (country_id LIKE %1$s OR name LIKE %1$s OR headline LIKE %1$s)'.$limit_query, secure($_GET['query'], 'search') )) or _error("SQL_ERROR");
                    if($get_rows->num_rows > 0) {
                        while($row = $get_rows->fetch_assoc()) {
                            $rows[] = $row;
                        }
                ...
                case 'edit':
                    ...
                    // get data
                    $get_data = $db->query(sprintf("SELECT * FROM regions WHERE id = %s ", secure($_GET['id'], 'int') )) or _error("SQL_ERROR");
                    if($get_data->num_rows == 0) {
                        _error(404);
                    }
                    $data = $get_data->fetch_assoc();
                    ...
                    // page header
                    page_header($control_panel['title']." &rsaquo; ".__("Regions")." &rsaquo; ".$data['name']);
                    break;
                ...
                case 'add':
                    ...
                    // page header
                    page_header($control_panel['title']." &rsaquo; ".__("Regions")." &rsaquo; Add");
                    break;
        ...
        case 'countries':
            ...
            switch ($_GET['sub_view']) {
                case '':
                    ...
                    $insights = [];
                    /* total regions */
                    $get_countries = $db->query("SELECT COUNT(*) as count FROM countries") or _error("SQL_ERROR");
                    $insights['countries'] = $get_countries->fetch_assoc()['count'];

                    // get data
                    require('includes/class-pager.php');
                    $params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
                    $params['total_items'] = $insights['countries'];
                    $params['items_per_page'] = $system['max_results'];
                    $params['url'] = $system['system_url'].'/'.$control_panel['url'].'/countries?page=%s';
                    $pager = new Pager($params);
                    $limit_query = $pager->getLimitSql();
                    $get_rows = $db->query("SELECT * FROM countries ORDER BY id ASC ".$limit_query) or _error("SQL_ERROR");
                    if($get_rows->num_rows > 0) {
                        while($row = $get_rows->fetch_assoc()) {
                            $rows[] = $row;
                        }
                ...
                case 'find':
                    ...
                    require('includes/class-pager.php');
                    $params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
                    $total = $db->query(sprintf('SELECT COUNT(*) as count FROM countries WHERE (country_id LIKE %1$s OR name LIKE %1$s OR headline LIKE %1$s) ORDER BY name ASC', secure($_GET['query'], 'search') )) or _error("SQL_ERROR");
                    $params['total_items'] = $total->fetch_assoc()['count'];
                    $params['items_per_page'] = $system['max_results'];
                    $params['url'] = $system['system_url'].'/'.$control_panel['url'].'/regions/find?query='.$_GET['query'].'&page=%s';
                    $pager = new Pager($params);
                    $limit_query = $pager->getLimitSql();
                    $get_rows = $db->query(sprintf('SELECT * FROM countries WHERE (country_id LIKE %1$s OR name LIKE %1$s OR headline LIKE %1$s)'.$limit_query, secure($_GET['query'], 'search') )) or _error("SQL_ERROR");
                    if($get_rows->num_rows > 0) {
                        while($row = $get_rows->fetch_assoc()) {
                            $rows[] = $row;
                        }
