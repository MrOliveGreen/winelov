...
class User
{
    ...
    public function get_post($post_id, $get_comments = true, $pass_privacy_check = false)
    {
        ...
        switch ($post['post_type']) {
            ...
            case 'article':
                ...
                if(!$post['article']['cover']){}
                else {
                    if(strpos($post['article']['cover'], 'vivino' ) !== false) $post['article']['parsed_cover'] = $post['article']['cover'];
                    else $post['article']['parsed_cover'] = get_picture($post['article']['cover'], 'article');
                }
     ...
     * add_rating
     *
     * @param integer $wine_id
     * @return array
     */
    public function has_review($wine_id)
    {
        global $db;

        /* insert the rating */
        $reviews = [];
        $has_review = $db->query("SELECT * FROM wine_rating WHERE wine_id=".$wine_id." AND user_id=".$this->_data['user_id']);
        if ($has_review->num_rows > 0) {
            while ($review = $has_review->fetch_assoc()) {
                $reviews[] = $review;
            }
        }
        return $reviews;
     ...
     * @return array
     */
    public function add_rating($wine_id, $post_id, $rating, $message)
    {
        global $db, $system, $date;

        /* default */
        $ratings = [];
        $ratings['user_id'] = $this->_data['user_id'];
        $ratings['wine_id'] = $wine_id;
        $ratings['post_id'] = $post_id;
        $ratings['rating'] = $rating;
        $ratings['review'] = $message;

        /* insert the rating */
        $db->query(sprintf("INSERT INTO wine_rating (wine_id, post_id, user_id, rating, review) VALUES (%s, %s, %s, %s, %s)", secure($ratings['wine_id'], 'int'), secure($ratings['post_id']), secure($ratings['user_id'], 'int'), secure($ratings['rating']), secure($ratings['review']))) or _error("SQL_ERROR_THROWEN");
     ...
     * @return array
     */
    public function _check_article($id)
    {
        global $db;

        $is_article = $db->query("SELECT * FROM posts_articles WHERE text LIKE '%".$id."%'");
        $post_id = [];
        if ($is_article->num_rows > 0) {
            while ($article = $is_article->fetch_assoc()) {
     ...
     * @return boolean
     */
    public  function delete_wine($wine_id)
    {
        global $db;
        // delete wine
        $result = $db->query(sprintf("DELETE FROM wines WHERE Id_item = %s", secure($wine_id, 'int'))) or _error("SQL_ERROR_THROWEN");
        return $result;
    }
     ...
     * delete winery
     *
     * @param string $winery_id
     * @return boolean
     */
    public  function delete_winery($winery_id)
    {
        global $db;
        // delete wine
        $result = $db->query(sprintf("DELETE FROM winery WHERE ID = %s", secure($winery_id, 'int'))) or _error("SQL_ERROR_THROWEN");
        return $result;
    }
    ...
    public  function delete_region($region_id)
    {
        global $db;
        // delete wine
        $result = $db->query(sprintf("DELETE FROM regions WHERE id = %s", secure($region_id, 'int'))) or _error("SQL_ERROR_THROWEN");
        return $result;
    }
     ...
     * @return array
     */
    public function get_wine_detail($id)
    {
        global $db;

        $get_wines = $db->query("SELECT * FROM wines WHERE Id_item = ".$id);
        $wines = [];
     ...
     * @return array
     */
    public function get_region_detail($name)
    {
        global $db;

        $get_regions = $db->query("SELECT * FROM regions WHERE name = '".$name."'");
        $regions = [];
        if ($get_regions->num_rows > 0) {
            while ($region = $get_regions->fetch_assoc()) {
                $regions = $region;
            }
        }
        return $regions;
     ...
     * @return array
     */
    public function get_region_related($region)
    {
        global $db;

        $get_wines = $db->query("SELECT * FROM wines WHERE region = '".$region."'");
        $wines = [];
        $i = 0;
        if ($get_wines->num_rows > 0) {
            while ($wine = $get_wines->fetch_assoc()) {
     ...
     * @return array
     */
    public function get_winery_detail($name)
    {
        global $db;

        $get_wineries = $db->query("SELECT * FROM winery WHERE Name = '".$name."'");
        $wineries = [];
        if ($get_wineries->num_rows > 0) {
            while ($winery = $get_wineries->fetch_assoc()) {
                $wineries = $winery;
            }
        }
        return $wineries;
     ...
     * @return array
     */
    public function get_winery_related($winery, $id)
    {
        global $db;

        $get_wines = $db->query("SELECT * FROM wines WHERE winery = '".$winery."'");
        $wines = [];
        $i = 0;
        if ($get_wines->num_rows > 0) {
            while ($wine = $get_wines->fetch_assoc()) {
                if($wine['Id_item'] == $id) continue;
                if($i>=15) break;
                $wines[] = $wine;
                $i++;
            }
     ...
     * @return array
     */
    public function get_wines($type, $grape, $region, $country, $style, $foodpair, $min, $max, $rating, $arrange, $number, $time)
    {
        global $db;
        if ($grape == '' && $region == '' && $country == '' && $style == '' && $foodpair == '') $where_query = '';
        else $where_query = 'WHERE ';
        if ($type != '') {
//            $where_query .= "type='" . $type . "' ";
        };
        if ($grape != '') {
            if ($where_query != 'WHERE ') $where_query .= 'AND ';
            $where_query .= "grapes LIKE '%" . $grape . "%' ";
        };
        if ($region != '') {
            if ($where_query != 'WHERE ') $where_query .= 'AND ';
            $where_query .= "region='" . $region . "' ";
        };
        if ($country != '') {
            if ($where_query != 'WHERE ') $where_query .= 'AND ';
            $where_query .= "country='" . $country . "' ";
        };
        if ($style != '') {
            if ($where_query != 'WHERE ') $where_query .= 'AND ';
            $where_query .= "wine_style='" . $style . "' ";
        }
        if ($foodpair != '') {
            if ($where_query != 'WHERE ') $where_query .= 'AND ';
            $where_query .= "foodpairing LIKE '%" . $foodpair . "%' ";
        }
        if ($where_query == '') $where_query = 'WHERE ';
        else $where_query .= 'AND ';
        if( $min !== '0' || $max !== '500+') {
            if($max == '500+') $where_query .= '( price > '.$min.' ) ';
            else $where_query .= '( price >= '.$min.' AND price <= '.$max.' )';
            $where_query .= ' AND ( rating > '.$rating.' AND rating != "" )';
        }
        else {
            $where_query .= ' ( rating > '.$rating.' AND rating != "" )';
        }
        $where_query .= ' ORDER BY ';
        switch ($arrange) {
            case 'high_rating':
                $where_query .= 'rating DESC';
                break;
            case 'discount':
                //        $where_query .= 'rating DESC';
                break;
            case 'hightolow':
                $where_query .= 'price DESC';
                break;
            case 'lowtohigh':
                $where_query .= 'price';
                break;
        ...
        if ($number != '') {
            $where_query .= ' LIMIT ' . $number;
            if ($time != 1) $where_query .= ' OFFSET ' . $number * ($time - 1);
        }
        $get_wines = $db->query("SELECT * FROM wines " . $where_query);
        $wines = [];
        if ($get_wines->num_rows > 0) {
            while ($wine = $get_wines->fetch_assoc()) {
                $wines[] = $wine;
