...
// valid inputs
if((!isset($_GET['post_id']) || !is_numeric($_GET['post_id'])) && (!isset($_GET['wine_id']) || !is_numeric($_GET['wine_id']))) {
	_error(400);
}
...
try {
	...
	switch ($_REQUEST['do']) {
		...
		case 'create':
			...
			if($_GET['wine_id']) {
			    $wine = $user->get_wine_detail($_GET['wine_id']);
                if(!$wine)  {
                    _error(404);
                }
                ...
                // return
                $return['share_publisher'] = $smarty->fetch("ajax.wine.share.publisher.tpl");
                $return['callback'] = "$('#modal').modal('show'); $('.modal-content:last').html(response.share_publisher); initialize_modal();";
                break;
            }
            break;
